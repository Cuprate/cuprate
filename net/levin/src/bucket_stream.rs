...
use crate::{LEVIN_SIGNATURE, PROTOCOL_VERSION};
...
#[pin_project]
pub struct MessageStream<D: MessageDecoder, S: AsyncRead + std::marker::Unpin> {
    #[pin]
    bucket_stream: BucketStream<S>,
    phantom: PhantomData<D>,
}

impl<D: MessageDecoder, S: AsyncRead + std::marker::Unpin> MessageStream<D, S> {
    pub fn new(stream: S) -> Self {
        MessageStream { 
