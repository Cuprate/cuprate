...
pub(crate) fn get_field_from_map<E: serde::de::Error>(value: &mut Value, field_name: &'static str) -> Result<Value, E> {
    value.get_and_remove(field_name).ok_or(serde::de::Error::missing_field(field_name))
}
...
pub(crate) fn get_internal_val_from_map<E, F, T>(value: &mut Value, field_name: &'static str, get_fn: F, expected_ty: &str) -> Result<T, E> 
where
    E: serde::de::Error,
    F: Fn(Value) -> Option<T>
{
