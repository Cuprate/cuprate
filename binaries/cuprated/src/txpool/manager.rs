...
/// # Panics
///
/// This function may panic if a critical service fails during startup.
...
impl TxpoolManagerHandle {
    ...
    pub async fn new_block(
        ...
        rx.await.map_err(|e| CupratedError::Txpool(e.into()))
    }
}
...
impl TxpoolManager {
    ...
    async fn remove_tx_from_pool(
        ...
        if remove_from_db {
            self.txpool_write_handle
                .ready()
                .await
                .map_err(|e| CupratedError::Txpool(e.into()))?
    ...
    /// recover from.
    #[instrument(level = "debug", skip_all, fields(tx_id = hex::encode(tx)))]
    async fn rerelay_tx(&mut self, tx: [u8; 32]) -> Result<(), CupratedError> {
        ...
        self.diffuse_service
            ...
            .await
            .map_err(|e| CupratedError::Txpool(anyhow::Error::from_boxed(e)))?;
    ...
    #[instrument(level = "debug", skip_all, fields(tx_id = hex::encode(tx)))]
    async fn handle_tx_timeout(&mut self, tx: [u8; 32]) -> Result<(), CupratedError> {
    ...
    #[instrument(level = "debug", skip_all, fields(tx_id = hex::encode(tx)))]
    async fn promote_tx(&mut self, tx: [u8; 32]) -> Result<(), CupratedError> {
    ...
    async fn new_block(&mut self, spent_key_images: Vec<[u8; 32]>) -> Result<(), CupratedError> {
