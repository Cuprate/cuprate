//! `cuprated` embeddable library.
//!
//! Call [`start`] to initialize and run the node. Returns a [`CuprateNode`]
...
//!
//! let node = cuprated::start(config).await;
...
use crate::{
    blockchain::SyncState, config::Config, constants::PANIC_CRITICAL_SERVICE_ERROR,
    tor::initialize_tor_if_enabled, txpool::IncomingTxHandler,
};

/// Handles returned by [`start`].
pub struct CuprateNode {
...
/// Initialize and start the Cuprate node.
///
/// Sets up thread pools, databases, P2P networking, the blockchain manager,
/// and RPC servers. Returns a [`CuprateNode`] with service handles.
///
...
pub async fn start(config: Config) -> CuprateNode {
    ...
    // Built before the blockchain manager so clearnet_interface can be moved.
    let node = CuprateNode {
