...
impl Node {
    ...
    /// # Errors
    ///
    /// Returns an error if `target_max_memory` is unresolved, the database
    /// is corrupt, or critical services fail to start.
    pub async fn launch(config: Config) -> Result<Self, anyhow::Error> {
        ...
        if tor_enabled {
            ...
            task_executor.spawn(async move {
                ...
                let Ok((tor_interface, tor_tx_handler_tx)) =
                    p2p::start_tor_p2p(&config, tor_context, &node_ctx)
                        .await
                        .inspect_err(|e| tracing::error!("Failed to start Tor P2P zone: {e:#}"))
                else {
                    return;
                };
