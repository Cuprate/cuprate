...
impl BlockchainManagerHandleSetter {
    pub fn set(self, tx: mpsc::Sender<BlockchainManagerCommand>) {
...
impl BlockchainManagerHandle {
    pub fn new() -> (Self, BlockchainManagerHandleSetter) {
        let once_lock = Arc::new(OnceLock::new());

        (
            BlockchainManagerHandle {
                tx: Arc::clone(&once_lock),
            },
            BlockchainManagerHandleSetter { tx: once_lock },
        )
    }
    ...
    pub async fn handle_incoming_block(
        ...
        let Some(incoming_block_tx) = self.tx.get() else {
    ...
    pub async fn pop_blocks(&self, numb_blocks: usize) -> Result<(), anyhow::Error> {
        let Some(manager_tx) = self.tx.get() else {
            // We could still be starting up the blockchain manager.
