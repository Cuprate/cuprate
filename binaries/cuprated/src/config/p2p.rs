...
impl SharedNetConfig {
        ...
        Self: FetchOwnAddr<Z>,
    {
        cuprate_address_book::AddressBookConfig {
            ...
            our_own_address: self.zone_own_addr(),
        }
    }
}

/// Helper trait for fetching the config field holding the address of this particular zone.
pub trait FetchOwnAddr<Z: NetworkZone> {
    fn zone_own_addr(&self) -> Option<Z::Addr>;
}

impl FetchOwnAddr<ClearNet> for SharedNetConfig {
    fn zone_own_addr(&self) -> Option<<ClearNet as NetworkZone>::Addr> {
        // [`ClearNet::BROADCAST_OWN_ADDR`] = `false`
        None
    }
}
