...
/// Split a [`u128`] value into low and high bits.
///
...
///
/// Note that the output of this function are `u64` representation of _bits_, not a numerical value.
///
/// See [`combine_low_high_bits_to_u128`] for the inverse function.
///
/// ```rust
/// # use cuprate_helper::map::*;
/// //                               regular u128 value          low         high
/// //                                                v            v         v
/// assert_eq!(split_u128_into_low_high_bits(u128::MAX), (u64::MAX, u64::MAX));
/// ```
pub const fn split_u128_into_low_high_bits(u: u128) -> (u64, u64) {
    let bits = u.to_le_bytes();
...
/// Combine 2 64-bit values into a single [`u128`].
///
/// The inputs:
...
///
/// Note that `low_bits` & `high_bits` are `u64` representation of _bits_, not a numerical value.
///
