...
impl PeerList {
    ...
    fn remove_peer_pruning_idx(&mut self, peer: &PeerListEntryBase) {
        let peer_list = self.pruning_idxs.get_mut(&peer.pruning_seed).expect("Pruning seed must exist if a peer has that seed");
        let mut i = 0;
...
mod tests {
    ...
    fn make_fake_peer_list(numb_o_peers: usize) -> PeerList {
        let mut peer_list = vec!{
            PeerListEntryBase::default(); numb_o_peers
        };
