use cuprate_address_book::{start_address_book, AddressBookConfig, AddressBookStore};
...
async fn test() {
    ...
    let mut book = start_address_book(PeerStore, config)
        .await
        .unwrap()
        .boxed_clone();
    ...
    let res = r_book
        .call(cuprate_address_book::AddressBookRequest::GetRandomWhitePeer(NetZone::Public))
        .await;
    println!("{:?}", res);
}
