...
impl Client {
    pub fn new(connection_info: Arc<ConnectionInfo>, server_tx:  mpsc::Sender<ClientRequest>) -> Self {
        Client { 
            connection_info, 
            server_tx
        }
...
impl tower::Service<InternalMessageRequest> for Client {
    ...
    fn poll_ready(&mut self, cx: &mut std::task::Context<'_>) -> std::task::Poll<Result<(), Self::Error>> {
        self.server_tx.poll_ready(cx).map_err(|e| PeerError::ClientChannelClosed) 
    }
