    ...
    PSync: PeerSyncSvc<N>,
{
    ...
    //
    // This does mean the every [TIMEOUT_INTERVAL] seconds we get a new peer list from the peer which seems like it may make
    // it easier for a node to fill a peers peerlist with junk peers however the node could just delay replies to messages
    // to get a timed sync request.
