...
impl CumulativeRctOutsCache {
    ...
    /// Returns the RCT output distribution for the request.
    pub fn distribution(
        &self,
        from_height: u64,
        ...
        cumulative: bool,
        chain_height: usize,
    ) -> Result<OutputDistributionData, tower::BoxError> {
        if to_height.is_some_and(|h| h.get() < from_height) {
            return Err("`to_height` is below `from_height`".into());
        }

        let to_height = to_height.map_or(chain_height - 1, |h| {
            let h = h.get();
            u64_to_usize(h)
        });
        ...
        let start_height = u64_to_usize(from_height).max(self.start_height);

        if start_height > to_height {
            return Ok(OutputDistributionData {
                amount: 0,
