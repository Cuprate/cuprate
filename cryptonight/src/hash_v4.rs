...
pub(crate) fn random_math_init(code: &mut [Instruction; NUM_INSTRUCTIONS_MAX + 1], height: u64) -> usize {
    ...
    const OP_ALUS: [usize; INSTRUCTION_COUNT] = [ALU_COUNT_MUL, ALU_COUNT, ALU_COUNT, ALU_COUNT, ALU_COUNT, ALU_COUNT];
    ...
    loop {
        ...
        let mut inst_data: [usize; 9] = [ 0, 1, 2, 3, 0xFFFFFF, 0xFFFFFF, 0xFFFFFF, 0xFFFFFF, 0xFFFFFF ];
        ...
        while ((latency[0] < TOTAL_LATENCY) || (latency[1] < TOTAL_LATENCY) || (latency[2] < TOTAL_LATENCY) || (latency[3] < TOTAL_LATENCY)) && (num_retries < 64) {
            ...
            if opcode == 5 {
                check_data(&mut data_index, 1, &mut data);
                opcode = if data[data_index as usize] >= 0 { InstructionList::Ror as u8 } else { InstructionList::Rol as u8 };
                data_index += 1;
            } else if opcode >= 6 {
                opcode = InstructionList::Xor as u8;
            } else {
                opcode = if opcode <= 2 { InstructionList::Mul as u8 } else { opcode - 2 };
            }

            let dst_index = (c >> INSTRUCTION_OPCODE_BITS) & ((1 << INSTRUCTION_DST_INDEX_BITS) - 1);
            let mut src_index = (c >> (INSTRUCTION_OPCODE_BITS + INSTRUCTION_DST_INDEX_BITS)) & ((1 << INSTRUCTION_SRC_INDEX_BITS) - 1);

            let a = dst_index as usize;
            let mut b = src_index as usize;

            // Don't do ADD/SUB/XOR with the same register
            if ((opcode == InstructionList::Add as u8) || (opcode == InstructionList::Sub as u8) || (opcode == InstructionList::Xor as u8)) && (a == b) {
                b = 8;
                src_index = 8;
            }
            ...
            // 2xXOR(a, b) = NOP
            if (opcode != InstructionList::Mul as u8) && ((inst_data[a] & 0xFFFF00) == ((opcode as usize) << 8) + ((inst_data[b] & 255) << 16)) {
                continue;
            }

            // Find which ALU is available (and when) for this instruction
            let mut next_latency = if latency[a] > latency[b] { latency[a] } else { latency[b] };
            let mut alu_index = -1;
            while next_latency < TOTAL_LATENCY {
                for i in (0..OP_ALUS[opcode as usize]).rev() {
                    if !alu_busy[next_latency][i] {
                        ...
                        if is_rotation[opcode as usize] && next_latency < (rotate_count * OP_LATENCY[opcode as usize]) {
                            continue;
                        }
            ...
            if next_latency <= TOTAL_LATENCY {
                ...
                asic_latency[a] = max(asic_latency[a], asic_latency[b]) + ASIC_OP_LATENCY[opcode as usize];
                ...
                if opcode == InstructionList::Add as u8 {
                    alu_busy[next_latency - OP_LATENCY[opcode as usize] + 1][alu_index as usize] = true;

                    check_data(&mut data_index, std::mem::size_of::<u32>(), &mut data);
                    code[code_size].c = u32::from_le_bytes([data[data_index] as u8, data[data_index+1] as u8, data[data_index+2] as u8, data[data_index+3] as u8]);
        ...
        while code_size < NUM_INSTRUCTIONS_MAX && asic_latency.iter().take(4).all(|&lat| lat < TOTAL_LATENCY) {
            ...
            let pattern = [InstructionList::Ror as u8, InstructionList::Mul as u8, InstructionList::Mul as u8];
