...
use super::{CRYPTONOTE_MAX_BLOCK_NUMBER, CRYPTONOTE_PRUNING_LOG_STRIPES, CRYPTONOTE_PRUNING_STRIPE_SIZE, CRYPTONOTE_PRUNING_TIP_BLOCKS};
...
impl PruningSeed {
    ...
    pub fn new(stripe: u32, log_stripes: u32) -> Result<PruningSeed, PruningError> {
        if !(log_stripes <= PRUNING_SEED_LOG_STRIPES_MASK) {
            ...
            Ok(PruningSeed(Some((log_stripes << PRUNING_SEED_LOG_STRIPES_SHIFT) | ( (stripe - 1) << PRUNING_SEED_STRIPE_SHIFT))))
        }
    }
    ...
    /// 
    pub fn get_next_unpruned_block(&self, block_height: u64, blockchain_height: u64) -> Result<u64, PruningError> {
        if block_height > CRYPTONOTE_MAX_BLOCK_NUMBER || block_height > blockchain_height {
            ...
            let seed_log_stripes = self.get_log_stripes().unwrap_or(CRYPTONOTE_PRUNING_LOG_STRIPES);
            let block_pruning_stripe = get_block_pruning_stripe(block_height, blockchain_height, seed_log_stripes).expect("We just checked if `block_height + CRYPTONOTE_PRUNING_TIP_BLOCKS >= blockchain_height`");
            ...
            let cycles_start = cycles + if seed_stripe > block_pruning_stripe {0} else {1}; 

            //                         amt_of_cycles  *         blocks in a cycle                            +   how many blocks through a cycles until the seed starts storing blocks 
            let calculated_height = cycles_start * (CRYPTONOTE_PRUNING_STRIPE_SIZE << seed_log_stripes) + (seed_stripe as u64-1) * CRYPTONOTE_PRUNING_STRIPE_SIZE;
    ...
    /// This function will also error if `block_height` > `blockchain_height`
    /// 
    pub fn get_next_pruned_block(&self, block_height: u64, blockchain_height: u64) -> Result<u64, PruningError> {
        let Some(seed_stripe) = self.get_stripe() else {
            // If the `get_stripe` returns None that means no pruning so the next 
...
fn get_block_pruning_stripe(block_height: u64, blockchain_height: u64, log_stripe: u32) -> Option<u32> {
    if block_height + CRYPTONOTE_PRUNING_TIP_BLOCKS >= blockchain_height {
        None
    } else {
        Some(
            (((block_height / CRYPTONOTE_PRUNING_STRIPE_SIZE) & ((1 << log_stripe) as u64 - 1))  + 1) as u32 // it's trivial to prove it's ok to us `as` here
        )
    }
}
...
#[cfg(test)]
mod tests {
    use crate::pruning::{PruningSeed, get_block_pruning_stripe};
    ...
    fn make_all_pruning_seeds() -> Vec<PruningSeed> {
        ...
        possible_stripes.map(|stripe| PruningSeed::new(stripe, CRYPTONOTE_PRUNING_LOG_STRIPES).unwrap()).collect()
    }
    ...
    fn blocks_pruning_stripe() {
        ...
        for i in 0_u32..8 {
            assert_eq!(get_block_pruning_stripe((i*4096) as u64, blockchain_height, CRYPTONOTE_PRUNING_LOG_STRIPES).unwrap(), i + 1);
        }

        for i in 0_u32..8 {
            assert_eq!(get_block_pruning_stripe(32768 + (i*4096) as u64, blockchain_height, CRYPTONOTE_PRUNING_LOG_STRIPES).unwrap(), i + 1);
        }

        for i in 1_u32..8 {
            assert_eq!(get_block_pruning_stripe(32767 + (i*4096) as u64, blockchain_height, CRYPTONOTE_PRUNING_LOG_STRIPES).unwrap(), i);
        }

        // Block shouldn't be pruned
        assert!(get_block_pruning_stripe(blockchain_height - 5500, blockchain_height, CRYPTONOTE_PRUNING_LOG_STRIPES).is_none());
    }
    ...
    fn next_unpruned_block() {
        ...
        for (i, seed) in all_valid_seeds.iter().enumerate() {
            assert_eq!(seed.get_next_unpruned_block(0, blockchain_height).unwrap(), i as u64 * 4096)
        }

        for (i, seed) in all_valid_seeds.iter().enumerate() {
            assert_eq!(seed.get_next_unpruned_block((i as u64 + 1) * 4096, blockchain_height).unwrap(), i as u64 * 4096  + 32768)
        }

        for (i, seed) in all_valid_seeds.iter().enumerate() {
            assert_eq!(seed.get_next_unpruned_block((i as u64 + 8) * 4096, blockchain_height).unwrap(), i as u64 * 4096  + 32768)
        }

        for (_, seed) in all_valid_seeds.iter().enumerate() {
            assert_eq!(seed.get_next_unpruned_block(76437863 - 1, blockchain_height).unwrap(),  76437863 - 1)
        }

        let zero_seed = PruningSeed(None);

        assert_eq!(zero_seed.get_next_unpruned_block(33443, 5565445).unwrap(), 33443);
